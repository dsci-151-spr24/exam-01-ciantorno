...
**The airlines with the most flights departing from NYC airports in 2013 are United Airlines, JetBlue Airways, ExpressJet Airlines, Delta Air Lines, and American Airlines.  For this question, I used the `count` function to make sure that the carriers were counted in the table, I used the `arrange(desc(n))` function to arrange the flights in decreasing order, and I used `inner_join` to combine the `airlines` and `flights` datasets and to make sure that I got accurate, complete, and organized data.**
